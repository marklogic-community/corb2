...
------
EXPORT_FILE_NAME (shared file to export output of com.marklogic.developer.corb.ExportBatchToFileTask - Not full path)
EXPORT-FILE-TOP-CONTENT (used by PostBatchUpdateFileTask to insert content at the top to EXPORT_FILE_NAME after batch process is complete, if it finds the text @URIS-BATCH-REF it replaces it by batch reference sent by URIS_MODULE)
