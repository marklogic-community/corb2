...
**<a name="XML-METADATA"></a>XML-METADATA** | An XPath to address the node that contains metadata portion of the XML. This must be different from the **XML-NODE**. The metadata is set as an external variable with name **METADATA** to **PRE-BATCH-MODULE** or **PROCESS-MODULE** if enabled by **METADATA-TO-PROCESS-MODULE**.
